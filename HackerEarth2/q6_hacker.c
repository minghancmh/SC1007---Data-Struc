...
void in2PreLL(char* infix, LinkedList *inExpLL)
{
    ...
    expressionQ(infix, fin);
    // ListNode* delHead = inExpLL->head;
    ...
    // printf("delHead point to %d\n", (*delHead).item);
    // delHead -> item = '?';

    // ListNode* iterLL = inExpLL->head;
    // LinkedList *fin = (LinkedList*) malloc(sizeof(LinkedList));
    // while(iterLL->item!='?'){
    
    //     insertNode(fin, iterLL->item,iterLL->type);
    //     iterLL = iterLL -> next;
    // }
    // printExpLL(*fin);
    // inExpLL -> head = fin -> head;


    // ListNode *fin = (ListNode*)malloc(sizeof(ListNode));
    // fin->item = 0;
    // fin -> type = OPT;
    // fin -> next = NULL;
    // iterLL->next = fin;

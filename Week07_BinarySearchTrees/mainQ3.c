...
int isBSTUtil(BTNode* node, BTNode* low, BTNode* high){
    //base
    if (node==NULL){return 1;} // a null tree is a BST

    if (low!=NULL && node -> item <= low->item){return 0;}
    if (high!=NULL && node-> item >= high -> item){return 0;}

    return isBSTUtil(node->left, low, node) && isBSTUtil(node->right, node, high);

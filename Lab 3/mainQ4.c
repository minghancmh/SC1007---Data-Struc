...
int smallestValue(BTNode *node){
    int small = node -> item;
    if (node->left!=NULL){if (smallestValue (node->left) < small){small = smallestValue(node->left);}}
    if (node->right!=NULL){if (smallestValue(node->right)<small){small = smallestValue(node->right);}}

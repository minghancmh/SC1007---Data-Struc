...
int BSTValidate(BTNode* root, BTNode* left, BTNode* right){
    if (root == NULL){return 1;} // null node is considered a BST

    if ((left!=NULL) && (root->item <= left->item)){return 0;}
    if ((right!=NULL) && (root->item >= right->item)){return 0;}

    return (BSTValidate(root->left, left, root) && BSTValidate(root->right, root, right));
}
...
int isBST(BTNode *root){
    ...
    return BSTValidate(root, NULL, NULL);

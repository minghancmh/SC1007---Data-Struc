...
void moveEvenItemsToBack(LinkedList *ll){
    ...
    ListNode* cur = ll->head;
    LinkedList* even = (LinkedList*) malloc(sizeof(LinkedList));
    LinkedList* odd = (LinkedList*) malloc(sizeof(LinkedList));  
    int oddI = 0;
    int evenI = 0;
    ...
    while (cur!=NULL){
        // printf("cur item: %d\n", cur->item);
        if ((cur->item)%2==1){
            insertNode2(odd, oddI, cur->item);
            oddI++;
        }
        else{
            insertNode2(even,evenI, cur->item);
            evenI++;
        }
        cur = cur->next;
    }
